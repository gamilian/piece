...
def process_image_pair(path, i, j, bg_color, training_dataset_path, gt_matrix):
    ...
    r_err_threshold, t_err_threshold = 4, 10
    for k in range(len(match_segments1)):
        ...
        err_trans = np.matmul(matrix, np.linalg.inv(gt_matrix))
...
def process_path(args):
    ...
    for _ in range(num_positives-1):
        noise_gt = add_uniform_noise_to_rigid_transform_2d(gt_matrix, 0.03, 2)
    ...
    for _ in range(num_positives//2):
        erro_matrix = add_erro_to_rigid_transform_2d(gt_matrix, 0.01, 0.06, 1, 2)
    ...
    #     erro_matrix = add_erro_to_rigid_transform_2d(gt_matrix, 0.1, 0.3, 4, 10)
    #     erro_item = FusionImage2(image1, image2, erro_matrix, bg_color)
    #     with global_idx.get_lock():
    #         image_path = os.path.join(training_dataset_path, "image", f"{global_idx.value}.png")
    #         global_idx.value += 1
...
if __name__ == '__main__':
    ...
    raw_dataset_path = glob.glob(os.path.join("/work/csl/code/piece/dataset/", "*_ex"))
    num_positives = 300
    num_negatives = 10
    num_works = 64
    # print(raw_dataset_path)
    training_dataset_path = '/work/csl/code/piece/dataset/12_15'
