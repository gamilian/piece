...
def FusionImage2(src, dst, transform, bg_color=[0,0,0]):
    ...
    f = np.where(b)
    src_roi_min_col = np.floor(np.min(d[0])).astype(int)
    src_roi_min_row = np.floor(np.min(d[1])).astype(int)
    src_roi_max_col = np.ceil(np.max(d[0])).astype(int)
    src_roi_max_row = np.ceil(np.max(d[1])).astype(int)
    
    dst_roi_min_col = np.floor(np.min(f[0])).astype(int)
    dst_roi_min_row = np.floor(np.min(f[1])).astype(int)
    dst_roi_max_col = np.ceil(np.max(f[0])).astype(int)
    dst_roi_max_row = np.ceil(np.max(f[1])).astype(int)
