...
write_lock = Lock()
# 进程池全局共享变量
global_idx = Value('i', -1)

def add_noise_to_rigid_transform_2d(matrix, angle_noise_mean, angle_noise_variance, translation_noise_mean, translation_noise_variance):
    '''
    不是标准刚性矩阵
    '''
    if matrix.shape != (3, 3):
    ...
    if np.random.randn() < 0:
        ...
        noisy_translation_vector = translation_vector + np.random.normal(-translation_noise_mean, translation_noise_variance, size=translation_vector.shape)
    else:
        noisy_rotation_angle = rotation_angle + np.random.normal(angle_noise_mean, angle_noise_variance)
        noisy_translation_vector = translation_vector + np.random.normal(translation_noise_mean, translation_noise_variance, size=translation_vector.shape)
...
def process_path(args):
    global global_idx
    path, i, j, gt_pose, bg_color, training_dataset_path, num_negatives = args
    ...
    if gt_item[1] < 0.0005:
        return
    noise_gt = add_noise_to_rigid_transform_2d(gt_matrix, 0.0045, 0.001, 0.6, 0.25)
    ...
    with open(os.path.join(training_dataset_path, "target.txt"), "a+") as f:
        if np.random.randn() < 0:
            cv2.imwrite(os.path.join(training_dataset_path, f"{global_idx.value}.png"), gt_item[0])
            f.write("0\n")
        else:
            cv2.imwrite(os.path.join(training_dataset_path, f"{global_idx.value}.png"), noise_item[0])
            f.write("0\n")

    for _ in range(num_negatives):
        erro_matrix = add_noise_to_rigid_transform_2d(gt_matrix, 0.4, 0.01, 5, 1)
        ...
        if not os.path.exists(training_dataset_path):
            os.makedirs(training_dataset_path)
        with open(os.path.join(training_dataset_path, "target.txt"), "a+") as f:
            cv2.imwrite(os.path.join(training_dataset_path, f"{global_idx.value}.png"), erro_item[0])
            f.write("1\n")

            
def create_dataset(raw_dataset_path, training_dataset_path, num_negatives=4, processes=16):
    args_list = []
    for path in raw_dataset_path:
        ...
        for i in range(num_fragments):
            for j in range(i + 1, num_fragments):
                args_list.append((path, i, j, gt_pose, bg_color, training_dataset_path, num_negatives))
...
if __name__ == '__main__':
    ...
    create_dataset(raw_dataset_path, training_dataset_path, num_negatives, num_works)