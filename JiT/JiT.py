...
class roi_pooling(nn.Module):
    def __init__(self, feature, output_size=(4, 4), spatial_scale=0.0625, sampling_ratio=0):
        super(roi_pooling, self).__init__()
        # 创建 ROI Align 层
        self.roi_align = RoIAlign(output_size, spatial_scale, sampling_ratio)
        self.fc = nn.Linear(feature, feature)

    def forward(self, x, cls_token, roi):
...
class PoolingTransformer(nn.Module):
    def __init__(self, image_size, patch_size, stride, base_dims, depth, heads,
                 mlp_ratio, num_classes=2, in_chans=3,
        ...
        # self.fc_dim = base_dims[-1] * heads[-1] * 4 * 4
        # self.fc1 = nn.Linear(self.fc_dim, 2048)
        # self.fc2 = nn.Linear(2048, 512)
        # self.head = nn.Linear(512, num_labels)
        # Classifier head
        self.roi_pooling = roi_pooling(self.embed_dim)
        self.fc1 = nn.Linear(base_dims[-1] * heads[-1], 256)
    ...
    def forward_features(self, x, roi):
        ...
        batch_index = torch.arange(roi.shape[0]).reshape(-1, 1).to(roi.device)

        # 将batch_index列拼接到RoIs Tensor上
        roi = torch.cat((batch_index, roi), dim=1)
      
        x, cls_tokens = self.transformers[-1](x, cls_tokens)
        x, cls_tokens = self.roi_pooling(x, cls_tokens, roi)
        # print(x.shape, cls_tokens.shape)
    ...
    def forward(self, x, roi):
        cls_token = self.forward_features(x, roi)
        cls_token = self.fc1(cls_token[:, 0])
...
class JigsawViT(nn.Module):
    ...
    def __init__(self, pretrained, num_labels=2, **kwargs):
        ...
        **kwargs) 
        ...
        if pretrained:
            state_dict = \
            torch.load('weights/jit_s_809.pth', map_location='cpu')
            model.load_state_dict(state_dict)
    ...
    def forward(self, x, roi):
        ...
        output = self.vit(x, roi)
        return output

if __name__=='__main__':
    image = preprocess_image("/data/csl/code/piece/dataset/MIT_ex/fragment_0001.png")
    model = JigsawViT(pretrained=False)

    rois = torch.tensor([[0.1, 0.2, 0.3, 0.4]], dtype=torch.float32)
    output = model(image, rois)
    print(output.shape)