...
def validate_loop(valid_dataloader, model, device):
    ...
    model.eval()
    with torch.no_grad():
        for valid_image, valid_label, valid_roi in tqdm(valid_dataloader):
            valid_image = valid_image.to(device)
            valid_label = valid_label.to(device)
            valid_roi = valid_roi.to(device)
            output = model(valid_image, valid_roi)
...
def train_loop(train_dataloader, valid_dataloader, model, device, epochs, batch_size, criterion, optimizer, lr_scheduler, writer, resume_checkpoint_dir):
    ...
    for epoch in range(epochs):
        ...
        for train_image, train_label, train_roi in tqdm(train_dataloader):
            train_image = train_image.to(device)
            train_label = train_label.to(device)
            train_roi = train_roi.to(device)
            # with autocast():
            output = model(train_image, train_roi)
